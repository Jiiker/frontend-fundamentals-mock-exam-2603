...
export function AvailableRoomList({ rooms, selectedRoomId, onSelect }: AvailableRoomListProps) {
  if (rooms.length === 0) {
    return (
      ...
      {rooms.map((room: { id: string; name: string; floor: number; capacity: number; equipment: string[] }) => {
        ...
        return (
          <div
            ...
            }; transition: all 0.15s; &:hover { border-color: ${isSelected ? colors.blue500 : colors.grey300}; }`}
          >
            <ListRow
              contents={
                <ListRow.Text2Rows
