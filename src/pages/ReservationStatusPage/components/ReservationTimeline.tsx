...
import { timeToMinutes } from 'shared/utils/common';
...
interface ReservationTimelineProps {
  ...
  reservations: { id: string; roomId: string; start: string; end: string; attendees: number; equipment: string[] }[];
}
    ...
    >
      ...
      >
        ...
        <div
          ...
          `}
        >
          {HOUR_LABELS.map((t: string) => {
            const left = ((timeToMinutes(t) - TIMELINE_START * 60) / TOTAL_MINUTES) * 100;
            return (
              <Text
                key={t}
                typography="t7"
            ...
            >
              {roomReservations.map(
                (res: { id: string; start: string; end: string; attendees: number; equipment: string[] }) => {
                  const left = ((timeToMinutes(res.start) - TIMELINE_START * 60) / TOTAL_MINUTES) * 100;
